...
ContactModel6DTpl<Scalar>::ContactModel6DTpl(
    ...
    // : Base(state, id, SE3::Identity(), type, nu, Vector2s::Zero()), // for croco/devel
    : Base(state, id, SE3::Identity(), nu, Vector2s::Zero()),
      xref_(xref),
      gains_(gains),
      type_(type) {}
...
ContactModel6DTpl<Scalar>::ContactModel6DTpl(
    ...
    const pinocchio::ReferenceFrame type)
    : Base(state, id, SE3::Identity(), Vector2s::Zero()),
      xref_(xref),
      gains_(gains),
      type_(type) {}
...
void ContactModel6DTpl<Scalar>::calcDiff(
  ...
  if (type_ == pinocchio::LOCAL_WORLD_ALIGNED || type_ == pinocchio::WORLD) {
    ...
    d->a0_temp_ = pinocchio::getFrameAcceleration(
    ...
    d->da0_dx.leftCols(nv).noalias() =
        d->lwaMl.toActionMatrix() * d->da0_dx_temp_.leftCols(nv);
    d->da0_dx.leftCols(nv).topRows(3).noalias() -=
        d->tmp_skew_lin_ * d->oRf * d->fJf.template bottomRows<3>();
    d->da0_dx.leftCols(nv).bottomRows(3).noalias() -=
        d->tmp_skew_ang_ * d->oRf * d->fJf.template bottomRows<3>();
    d->da0_dx.rightCols(nv).noalias() =
        d->lwaMl.toActionMatrix() * d->da0_dx_temp_.rightCols(nv);
  }
}
